...
from utils import save_prot_pipeline, load_prot_pipeline

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))
from NeuralNetClasses import AutoencoderTransformer, AutoencoderTransformerProt
from Preprocessors import binarize_cci, clipper_func
...
if config_arg.ratio:
    data_cc = pd.read_csv(f'{datadir}/transfer_all_cc_prot_ratio_{config_arg.ratio}.csv')
else:
    data_cc = pd.read_csv(f'{datadir}/transfer_all_cc_prot.csv')
...
data_all = data_cc.merge(data_prot, on='eid', how='left', validate='m:1')
...
for cn in comp_loop:
    ...
    for dataset in ['infl_1', 'infl_1+2', 'prot_all']:
        ...
        if dataset != 'prot_all':
            ...
            raise ValueError(f"Unknown dataset name: {dataset}")
        columns_base = ['case', 'age_surgery', 'sex', 'score']
        data_now_selected = data_now[columns_base + selected_columns].copy()
            ...
            if config_arg.config_fitfinal.lower() == 'no':
                ...
                if run_ae == True:
                    if config_arg.ratio:
                        ...
                        results_filename = f'results_{cn}_{name}_{args[0]}_{dataset}_ae_16_False_nonsurg_newcontrols_newprepro.csv'
                else:
                    if config_arg.ratio:
                ...
                print('Best parameters for final model:', imba_pipeline.get_params())
                ...
                # save model
                save_prot_pipeline(final_model, model_file_path)
                # save best params
                with open(param_file_path, 'w') as param_file:
                    yaml.dump(imba_pipeline.get_params(), param_file)
                ...
                save_prot_pipeline(model_payload, model_file_path_payload)
